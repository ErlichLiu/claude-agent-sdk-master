...
export interface PromaAgentConfig {
  ...
  model?: string;                      // 模型（默认：claude-sonnet-4-6）
...
### 8. Agent Teams Auto-Resume 机制

#### 问题背景

Agent Teams 中 teammates 通过 inbox 文件 (`~/.claude/teams/{team}/inboxes/team-lead.json`) 返回工作结果，
但 SDK 的 `query()` generator 在 `complete` 事件后即结束，team lead 无法在同一轮中读取 inbox 并生成汇总。

#### 解决方案：延迟 Result + Inbox 轮询 + 流式 Resume

**核心文件**：`app/api/chat/route.ts`、`components/chat-interface.tsx`

**SSE 事件流**（teams 活跃时）：
```
task_started (×N) → task_notification (×N) → complete
  → [result SSE 被延迟] → waiting_resume SSE
  → 轮询 inbox (5×2s) → resume_start SSE → content SSE (流式) → result SSE
```
...
**关键函数**：
- `pollInboxWithRetry()` — 带重试的 inbox 轮询（5 次 × 2 秒间隔）
- `formatSummaryFallbackPrompt()` — 当 inbox 为空时用 task summaries 构造 fallback prompt
- `handleAgentEvent()` — 新增 `deferResult` 参数，teams 活跃时不发送 result SSE

**前端处理**：
- `currentAssistantIdRef` — ref 跟踪当前活跃的 assistant message ID
- `waiting_resume` 事件 — 保持 streaming 状态
- `resume_start` 事件 — 结束当前 message，创建新的 streaming message
- `content` 事件 — 写入 ref 指向的当前 message（支持 resume 流式输出）

**Watchdog（安全网）**：5 秒间隔检查 idle workers，防止 inbox 死锁。
